...
\usepackage{templates/mystyle2015}			% Indeed, my project styles
\usepackage{templates/mySpecialTokens}		% Definition of tokens used in modal logic, formal semantics and term rewriting
\usepackage{templates/myarrows}				% Some few specific arrow symbols used throughout the document, based on extpfeil
...
\usepackage[utf8]{inputenc}			% NOT required when using xetex as opposed to pdftex
...
%%%%%%%%%%%%%%%
%% Theorems
%%				based on <ntheorem> package
									% Use of new theorems package <ntheorem> makes easier handling of theorem-like environments; refer to ftp://ftp.dante.de/pub/tex/macros/latex/contrib/ntheorem/ntheorem.pdf
\usepackage{amsmath}				% <amsmath> needs to be loaded before <ntheorem> when the latter's [amsmath] option is used (section 3.2.1, ftp://ftp.dante.de/pub/tex/macros/latex/contrib/ntheorem/ntheorem.pdf) 

%\usepackage[ntheorem]{empheq}		% The <mathtools> and <empheq> packages should be loaded before <ntheorem>; this loads <amsmath> as well

%\usepackage[thmmarks]{ntheorem}	% DESPITE section 3.2.1 of ntheorem.pdf above, DO NOT USE MORE THAN ONE \usepackage[...]{ntherem} DEFINITION; 
%\usepackage[amsmath,thmmarks]{ntheorem}	% this will result in ! LaTeX Error: Option clash for package ntheorem.
\usepackage[amsmath,amsthm,thmmarks]{ntheorem}		% option [amsthm] has to be used instead of \usepackage{amsthm}
%\usepackage[thref]{ntheorem}		% There is no good reason to use Option [thref] when using <cleveref> (see below), since the [thref] option is superseded by <cleveref>.
									% Option [thref] interferes with the <babel> package, thus in case that <babel> is used, <ntheorem> has to be loaded after <babel>.
%% End of Theorems
%%%%%%%%%%%%%%%%
\usepackage{cases}					% To make separate labels in cases: https://tex.stackexchange.com/questions/31951/separate-labels-in-cases. It appears that <cases> needs to be loaded after <amsmath>
\usepackage{mdframed}
...
\usepackage{cleveref}				% sourced from http://tug.ctan.org/macros/latex/contrib/cleveref/cleveref.pdf Makes varioref clever, and reuses its work to improve its own workings. 
									...
									% Therefore, to be safe, ***we declare it AS LAST PACKAGE in the document's preamble***.
...
%%% ------- mijn configs: --------- %%%
%
%% shortcuts and abbrevs
%

\def\bc#1{\multicolumn{2}{c}{#1}}		% shorter, hence easier to fit the table
\def\br#1{\multirow{2}{*}{#1}}			% shorter, hence easier to fit the table
...
%\newcommand*\ntsa{\ensuremath{\varnothing}}	% no transcription allowed: empty set
\newcommand*\ntsa{\ensuremath{\top}}	% no transcription allowed: verum
